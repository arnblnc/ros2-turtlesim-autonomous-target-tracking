...
class TurtleControllerNode(Node):
    def __init__(self):
        ...
        self.pose_: Pose = None # Hinting the variable type is Pose
    ...
    def callback_alive_turtles(self, msg: TurtleArray):
        if len(msg.turtles) > 0:
            if self.catch_closest_turtle_first_:
                ...
                for turtle in msg.turtles: # use pythagoras theory to find distance
                    dist_x = turtle.x - self.pose_.x
                    dist_y = turtle.y - self.pose_.y
                    distance = math.sqrt(dist_x * dist_x + dist_y * dist_y)
                    if closest_turtle_distance == None or distance < closest_turtle_distance: # checking for closest turtle
                        closest_turtle = turtle
                        closest_turtle_distance = distance
                self.turtle_to_catch_ = closest_turtle
            else:
                self.turtle_to_catch_ = msg.turtles[0] # Catch the first turtle in the list
    ...
    def control_loop(self):
        ...
        distance = math.sqrt(dist_x * dist_x + dist_y * dist_y) # distance to target
        ...
        if distance > 0.5:
            ...
            # orientation
            goal_theta = math.atan2(dist_y, dist_x) # use inverse of tan to find angle
            diff = goal_theta - self.pose_.theta
            if diff > math.pi: # normalise angle
                diff -= 2*math.pi
            elif diff < -math.pi:
                diff += 2*math.pi
            ...
            # target reached
            cmd.linear.x = 0.0
            cmd.angular.z = 0.0 # only rotates in z-axis due to right-hand rule
